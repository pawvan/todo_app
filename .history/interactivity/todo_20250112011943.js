...
function getRandomColor() {
  // We'll stick with just the gray or blue shades for now
  const colors = ["#ccc", "#0099cc"]; // Gray and blue
...
function addTask() {
  ...
  if (taskText !== "") {
    ...
    wrapperCheck.style.borderColor = getRandomColor(); // Apply random color (gray or blue)
...
function loadTasks() {
  ...
  tasks.forEach((task) => {
    ...
    // Apply same random color logic for loaded tasks
    wrapperCheck.style.borderColor = getRandomColor();
    wrapperCheck.appendChild(completeCheckbox);
